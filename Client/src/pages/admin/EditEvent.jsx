...
import { useUpdateEvent } from '../../hooks/queries/useAdmin';
...
export default function EditEvent() {
  ...
  const handleSubmit = async (e) => {
    ...
    updateEventMutation.mutate({ eid: id, eventData: dataToSend }, {
      onSuccess: () => {
    ...
    <div className="max-w-6xl mx-auto selection:bg-violet-500/30 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <h1 className="font-bebas text-7xl text-white tracking-wider">EDIT <span className="text-blue-500">EVENT</span></h1>
          <p className="text-gray-500 font-medium">Update event parameters and visibility.</p>
        </div>
        
        <div className="flex items-center gap-4 px-6 py-3 bg-white/5 border border-white/5 rounded-2xl">
           <Zap className="w-4 h-4 text-blue-500" />
           <span className="text-white text-[10px] font-black uppercase tracking-widest">Update Protocol</span>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 md:p-16 space-y-20">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
            <div className="lg:col-span-4">
               ...
               <p className="text-gray-600 text-xs font-medium leading-relaxed">Modify the title, artist, and primary classification.</p>
            </div>
            <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Event Title</label>
                <input required name="title" value={formData.title} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Headlining Artist</label>
                <input required name="eventArtistName" value={formData.eventArtistName} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Event Category</label>
                <select name="category" value={formData.category} onChange={handleChange} className="w-full bg-[#0F0F16] border border-white/5 rounded-2xl px-6 py-4.5 text-gray-500 outline-none focus:border-blue-500/50 transition-all font-bold appearance-none cursor-pointer">
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-gray-500 outline-none focus:border-blue-500/50 transition-all font-bold appearance-none"
                >
              ...
              <div className="md:col-span-2 space-y-4">
                <div className="flex items-center gap-4 p-4 bg-white/[0.02] border border-white/5 rounded-2xl">
                   ...
                   />
                   <div>
                     <p className="text-white text-xs font-bold uppercase tracking-tight">Public Visibility</p>
                     <p className="text-gray-700 text-[10px] font-black uppercase tracking-widest">Enable to show on homepage discovery</p>
                   </div>
                </div>
              </div>
              <div className="md:col-span-2 space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Event Description</label>
                <textarea required name="description" value={formData.description} onChange={handleChange} rows="6" className="w-full bg-white/[0.02] border border-white/5 rounded-3xl px-6 py-6 text-white outline-none focus:border-blue-500/50 transition-all font-bold resize-none" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 pt-10 border-t border-white/5">
            ...
            <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="space-y-2">
                ...
                <input required type="date" name="eventDate" value={formData.eventDate} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Venue</label>
                <input required name="eventLocation" value={formData.eventLocation} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Duration</label>
                <input required name="duration" value={formData.duration} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Total Seats</label>
                <input required type="number" name="totalSeats" value={formData.totalSeats} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
              <div className="space-y-2">
                <label className="text-gray-700 text-[10px] font-black uppercase tracking-widest ml-1">Entry Price (₹)</label>
                <input required type="number" name="ticketPrice" value={formData.ticketPrice} onChange={handleChange} className="w-full bg-white/[0.02] border border-white/5 rounded-2xl px-6 py-4.5 text-white outline-none focus:border-blue-500/50 transition-all font-bold" />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-10 border-t border-white/5">
             ...
             <motion.button
               ...
               type="submit"
               className="bg-white text-black px-12 py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] flex items-center gap-3 shadow-2xl hover:bg-blue-600 hover:text-white transition-all duration-500 disabled:opacity-50 group"
             >
